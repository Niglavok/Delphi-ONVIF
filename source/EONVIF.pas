...
function ONVIFGetDeviceInformation(const Addr, EPort, ETimeout, UserName, Password: String): String; // Changed by Niglavok
...
function ONVIFGetProfiles(const Addr, EPort, ETimeout, UserName, Password: String): String; // Changed by Niglavok
...
type
  ...
  // Addr -> http://<host>/onvif/Media
function ONVIFGetStreamUri(const Addr, EPort, ETimeout, UserName, Password, Stream, Protocol, ProfileToken: String): String; // Changed by Niglavok
...
type
  ...
  // Addr -> http://<host>/onvif/Media
function ONVIFGetSnapshotUri(const Addr, EPort, ETimeout, UserName, Password, ProfileToken: String): String; // Changed by Niglavok
...
// ------------------------
//
procedure ONVIFRequest(const Addr, EPort, ETimeout: String; const InStream, OutStream: TStringStream); overload; // Changed by Niglavok
procedure ONVIFRequest(const Addr, EPort, ETimeout, Request: String; Var Answer: String); overload; // Changed by Niglavok
//
// ------------------------
//
...
function IfNumbers(const S: string): Boolean;                                   // Added by Niglavok
var                                                                             // Added by Niglavok
  I : Cardinal;                                                                 // Added by Niglavok
begin                                                                           // Added by Niglavok
  Result := True;                                                               // Added by Niglavok
  for I := 1 to Length(S) do                                                    // Added by Niglavok
    if (S[I] in ['0'..'9']) = False then begin                                  // Added by Niglavok
      Result:=False;                                                            // Added by Niglavok
      Break;                                                                    // Added by Niglavok
    end;                                                                        // Added by Niglavok
end;
procedure Register;
begin
...
procedure ONVIFRequest(const Addr, EPort, ETimeout, Request: String; Var Answer: String); // Changed by Niglavok
Var
...
begin
  ...
  try
    ONVIFRequest(Addr, EPort, ETimeout, InStream, OutStream); // Changed by Niglavok
...
procedure ONVIFRequest(const Addr, EPort, ETimeout: String; const InStream, OutStream: TStringStream); // Changeg by Niglavok
Var
  idhtp1: TIdHTTP;
  Uri: TIdURI;
begin
  ...
  //
  if IfNumbers(EPort) then begin                                                // Added by Niglavok
    idhtp1.URL.Port := EPort;                                                   // Added by Niglavok
  end else begin                                                                // Added by Niglavok
    idhtp1.URL.Port := '80';                                                    // Added by Niglavok
  end;                                                                          // Added by Niglavok
  if IfNumbers(ETimeout) then begin                                             // Added by Niglavok
    idhtp1.ConnectTimeout := StrToInt(ETimeout);                                // Added by Niglavok
    idhtp1.ReadTimeout := StrToInt(ETimeout);                                   // Added by Niglavok
  end else begin                                                                // Added by Niglavok
    idhtp1.ConnectTimeout := 5000;                                              // Added by Niglavok
    idhtp1.ReadTimeout := 5000;                                                 // Added by Niglavok
  end;                                                                          // Added by Niglavok
  //
  Uri := TIdURI.Create(Addr);
  try
    try                                                                         // Added by Niglavok
    With idhtp1 do
    begin
      AllowCookies := True;
    ...
    except // Catching Connection Exceptions                                    // Added by Niglavok
      on E: EIdException do begin                                               // Added by Niglavok
        // An network error occurred while trying to connect                    // Added by Niglavok
      end;                                                                      // Added by Niglavok
      on E: Exception do begin                                                  // Added by Niglavok
        // An unknown error occurred while trying to connect                    // Added by Niglavok
      end;                                                                      // Added by Niglavok
    end;                                                                        // Added by Niglavok
  finally
    Uri.Free;
...
begin
  // Addr -> http://<host>/onvif/device_service
  ONVIFRequest(Addr, EPort, ETimeout, PrepareGetDeviceInformationRequest(UserName, Password), Result); // Changed by Niglavok
end;
function XMLDeviceInformationToDeviceInformation(const XMLDeviceInformation: String; Var DeviceInformation: TDeviceInformation): Boolean;
var
  SS: TStringStream;
...
begin
  // Addr -> http://<host>/onvif/Media
  ONVIFRequest(Addr, EPort, ETimeout, PrepareGetProfilesRequest(UserName, Password), Result); // Changed by Niglavok
end;
function XMLProfilesToProfiles(const XMLProfiles: String; Var Profiles: TProfiles): Boolean;
var
  SS: TStringStream;
...
begin
  // Addr -> http://<host>/onvif/Media
  ONVIFRequest(Addr, EPort, ETimeout, PrepareGetSnapshotUriRequest(UserName, Password, ProfileToken), Result); // Changed by Niglavok
end;
function XMLSnapshotUriToSnapshotUri(const XMLSnapshotUri: String; Var SnapshotUri: TSnapshotUri): Boolean;
var
  SS: TStringStream;
...
begin
  // Addr -> http://<host>/onvif/Media
  ONVIFRequest(Addr, EPort, ETimeout, PrepareGetStreamUriRequest(UserName, Password, Stream, Protocol, ProfileToken), Result); // Changed by Niglavok
end;
function PrepareGetStreamUriRequest(const UserName, Password, Stream, Protocol, ProfileToken: String): String;
const
